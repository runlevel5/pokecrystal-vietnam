...
The mail composition character input screen (`data/text/mail_input_chars.asm`) supports **full Vietnamese characters** with automatic translation for link cable compatibility.

**How it works:**
- Mail messages can be written using Vietnamese characters (3 pages of characters)
...
- Mail received from English Crystal displays correctly using the English font (no translation needed)

**Implementation:**
- File: `data/text/mail_input_chars.asm`
- Layout: 3 pages of Vietnamese characters
...
- Font switching: `engine/pokemon/mail_2.asm:ReadMailMessage` loads appropriate font based on nationality
...
4. `ConvertVietnameseMailToEnglish` translates message and author name to base English letters
5. English Crystal receives readable base-letter text

**Incoming Mail Display (EN → VN):**
1. English Crystal sends mail (any language: EN, FR, DE, IT, ES)
2. `ParseMailLanguage` returns appropriate language code based on nationality
3. `ReadMailMessage` in `mail_2.asm` loads the correct font for that language
4. No character translation needed - the font handles display correctly

**Font Loading by Nationality:**
| Nationality | Font Loaded |
|-------------|-------------|
| "VN" | Vietnamese `Font` |
| "EF", "EG" | `FrenchGermanFont` |
| "EI", "ES" | `SpanishItalianFont` |
| Other | `StandardEnglishFont` |

**Note:** Pokemon and trainer name input (`data/text/name_input_chars.asm`) also supports full Vietnamese characters, handled by the translation layer during link cable trading.
...
- Pokemon nicknames use **full Vietnamese characters** (5-page input like player names)
- Nicknames are automatically translated to base English letters when trading with English Crystal (see "Link Cable Trading Translation Layer" section)
...
The naming screen uses the same 5-page Vietnamese character set for all naming types:

### Character Input Pages (data/text/name_input_chars.asm)

| Naming Type | Character Set | Pages | Notes |
|-------------|---------------|-------|-------|
| Player name | Vietnamese (NameInputPage1-5) | 5 | Full Vietnamese with diacritics |
| Rival name | Vietnamese (NameInputPage1-5) | 5 | Full Vietnamese with diacritics |
| Mom's name | Vietnamese (NameInputPage1-5) | 5 | Full Vietnamese with diacritics |
| Friend name | Vietnamese (NameInputPage1-5) | 5 | Full Vietnamese with diacritics |
| Pokemon nickname | Vietnamese (NameInputPage1-5) | 5 | Translated to English when trading via link cable |
| PC Box name | Vietnamese (NameInputPage1-5) | 5 | Box names are NOT transmitted over link cable |

### Pokemon Nickname Translation for Link Cable

Pokemon nicknames can now use **full Vietnamese characters** with automatic translation for link cable compatibility:

**How it works:**
1. Player names their Pokemon using Vietnamese characters (e.g., "Rồng", "Điện")
2. In Vietnamese game: Displays as "Rồng", "Điện" with accents
3. When trading to English game: Automatically translated to "Rong", "Dien" (accents stripped)
4. When trading VN↔VN: Preserves all accented characters

**Implementation:**
- Input: 5-page Vietnamese keyboard (same as player name input)
- Translation: `Link_FixDataForPeerLanguage` in `engine/link/link.asm` calls `TranslateString_PartyMonNicknames`
- Peer detection: Uses `wPeerLanguage` to conditionally translate only for English peers
...
**Player/Rival/Mom/Friend Names:** These names are also translated via the link cable translation layer when trading with English Crystal.
...
**Layer 1 — 2-byte RN signature:** `LANG_VN_BYTE1` (`$55`) followed by `LANG_VN_BYTE2` (`$AA`) are embedded as the first two random bytes in the RN data section. The receiver skips any `$FD` preamble bytes in the received buffer (`wOTLinkBattleRNData`), then checks the first non-`$FD` byte pair for the 2-byte signature. The preamble-skip is necessary because `wOTLinkBattleRNData` contains both preamble and RN data, and variable sync timing can shift the boundary.

**Layer 2 — Player name field backup:** A 2-byte signature `$55, $AA` (`LANG_VN_BYTE1` + `LANG_VN_BYTE2`) is embedded at offsets 9-10 of the `NAME_LENGTH` (11-byte) player name field. Player names are at most `PLAYER_NAME_LENGTH` (8 bytes including `$50` terminator), so these trailing bytes are unused by English Crystal. After receiving party data, the backup check confirms or corrects the RN-based detection. This mirrors the technique used by European G/S/C for mail nationality detection (see `european_mail.asm`).

- **VN ↔ VN**: Both peers detect the signature, preserve Vietnamese accented characters
...
3. `FixDataForLinkTransfer` sets up RN preamble (all $FD bytes) and places 2-byte signature (`$55`, `$AA`) as the first two random bytes
4. RN exchange happens via `Serial_ExchangeBytes`
5. Skip `$FD` preamble bytes in received buffer, then check first non-`$FD` byte pair for `$55 $AA` → set wPeerLanguage
